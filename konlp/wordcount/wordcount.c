...
ULONG set_delimiter(UCHAR *text, int flag_lineinput, int flag_dots)
{
	...
	while (*p && p-text < N_bytesize) {
		if (flag_lineinput) {	/* ???? ?????? ??? --> not ???? ???? */
			if (flag_ignore_empty_line) {	// 2013/07/02 -- ???��? ????: line count?? ?? ???? ???��? ????????...
...
//			while (*p && !(*p == 0x0D || *p == 0x0A)) p++;
			...
			*p++ = '\0'; n++;
			if (flag_dots && (n % 100000 == 0)) fputc('.', stderr);	// ???? ??? ???
		} else {
...
int insert_word_or_count(ULONG nth, UCHAR *aWord, WORDCNT_STR *wp)
{
	int i=wp->root, n=wp->n_uniqwords, res;
		...
		while (i >= 0) {
			...
			if (!res) {		// ÀÌ¹Ì ÁžÀçÇÏŽÂ °Í --> countžž Áõ°¡
				wp->count[i]++;
				return wp->root;
			} else if (res < 0) {
				if (wp->llink[i] == -1) {
		...
		wp->words[n] = aWord;
		wp->count[n] = 1;
		wp->llink[n] = wp->rlink[n] = -1;	// llink, rlink ÃÊ±âÈ­
		wp->n_uniqwords = n+1;
	
		return wp->root;	// skewed tree¿¡ ŽëÇÑ Ã³ž®°¡ ÇÊ¿äÇÔ.
}
...
int sort_and_count(WORDCNT_STR *wp, int flag_lineinput, int flag_dots)
{
	...
	for (i=0; i < wp->n_words; i++) {
		if (flag_lineinput) {
			if (!flag_ignore_empty_line)	// 2013/07/02 -- °­œÂœÄ ŒöÁ€: line countÇÒ ¶§ °ø¹é ¶óÀÎµµ °è»êÇÏµµ·Ï...
				;
		...
		if (*p == 0xA1 && *(p+1) == 0xA1) p += 2;	// 2¹ÙÀÌÆ® °ø¹é ¹®ÀÚ

		wp->root = insert_word_or_count(i, p, wp);
		p += strlen((char*)p) + 1;

		if (flag_dots && (i % 10000 == 0)) fputc('.', stderr);		// ÁøÇà »óÈ² Ç¥œÃ
	}
	// fputc('\n', stderr);
...
/*
	...
	<????> dynamic memory?? ?????????? free ????? ?? --> ??? free_memory()
*/
int load_words_and_links(char *fname_word, char *fname_link, WORDCNT_STR *wp)
{
	...
	ULONG i, n;
	UCHAR *p;
	int flag_lineinput=1;	// ????? wordlist?? ?? line?? 1????????(line count?? ??��)
	int flag_dots=0;	// ???????? ... ???? ???
...
int word_count(char *infile, WORDCNT_STR *wp, int flag_lineinput, int flag_dots)
{
	...
	if (load_words_and_links(WORD_FILE_NAME, LINK_FILE_NAME, wp));	// ±âÁž word count ³»¿ëÀ» ÀÐŸîµéÀÓ
	else {
		wp->root = -1;	// root node ÃÊ±âÈ­
		wp->n_uniqwords = 0;
		if (!memory_allocation(wp, n)) return 0;
	}
	wp->n_uniqwords = sort_and_count(wp, flag_lineinput, flag_dots);	// ÀÔ·Â ÆÄÀÏ¿¡ ŽëÇÑ word count
	n_sort_index = 0;
	set_sorting_index(wp->root, wp);	// Sorting Index 'sidx[]' °ª setting
	return wp->n_uniqwords;
}

/*
	???? ?? ??? ????? ??????? ???? old version.
	??, load_words_and_links()?? ??????? ???, ???? ??? ????? ??????? ?? ???.
*/
